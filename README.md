...
- 日期：报告快照的 1900/1904 日期系统，不将合法日期系统判为故障，不猜测日期值或修改序号。
- 每项都有判断和完成状态。文件 API 不可用时保留公式结果；中断、损坏元数据、超出限制会说明原因，不能视为全面通过。
- 最多读取 100 万个已使用区域内的单元格，每批最多 5,000 个；详情最多保留 5,000 条，页面显示前 100 条。超限明确显示省略数量，下载也受相同详情上限限制。一格可能产生多条发现。
- **下载详细兼容报告**包含工作表名、地址、公式与引用等业务内容，仅本地下载，分享前自行核对。原有简要诊断仍只导出能力、计数和错误码。开始其他操作或重载窗格会清空旧报告，避免误用旧结果。
